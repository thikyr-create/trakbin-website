...
export function MobileMenu() {
  ...
  return (
    <div className="lg:hidden">
      ...
      {open && (
        <div className="fixed inset-x-0 bottom-0 top-16 z-40 bg-forest-950/95 px-6 py-10 backdrop-blur-lg">
          <nav className="flex flex-col gap-5">
            {NAV_LINKS.map((l) => (
              <Link key={l.href} href={l.href} onClick={() => setOpen(false)}
                className="font-display text-2xl font-bold text-white">{l.label}</Link>
            ))}
