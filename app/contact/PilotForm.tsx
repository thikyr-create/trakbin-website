    ...
    <form onSubmit={submit} className="rounded-2xl border border-line bg-card p-8 card-shadow sm:p-10">
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2">
        <div><label className={label} htmlFor="name">Name</label>
          <input id="name" required className={input} value={v.name} onChange={set("name")} placeholder="Your name" /></div>
        <div><label className={label} htmlFor="org">Organization</label>
          <input id="org" required className={input} value={v.org} onChange={set("org")} placeholder="Operator / property / agency" /></div>
      </div>
      <div className="mt-5"><label className={label} htmlFor="email">Email</label>
        <input id="email" type="email" required className={input} value={v.email} onChange={set("email")} placeholder="you@organization.com" /></div>
      <div className="mt-5"><label className={label} htmlFor="message">What do you operate or manage?</label>
        <textarea id="message" rows={5} required className={input} value={v.message} onChange={set("message")}
          placeholder="Fleet size, buildings, zones, current process…" /></div>
      <button type="submit"
